...
- 相册照片统一预处理后再入库（脚本 `scripts/watermark.py`，参数都在文件顶部）：转 JPG、长边压到 1600、清空 EXIF（含 GPS）、打两层水印——画面下三分之一一行斜向虚化大字（黑色 5%）+ 右下角小字网址（白色 50% 带阴影），文字都是 `agnesv1.github.io`。依赖 Pillow + pillow-heif（HEIC 必须转，浏览器不认）。原图另存在仓库外，库里只放处理过的。
